...
ct.setSelectionRange = function (ta, start, end) {
	if (ta.setSelectionRange) {
		...
		let scrollTop = ta.scrollHeight;
		ta.value = fullText;
		let textareaHeight = ta.clientHeight;
		ta.focus();
...
ct.hlink = function (toWhat, text) {
	let wgServer = mw.config.get('wgServer') || 'http://en.wikipedia.org';
	let wgArticlePath = mw.config.get('wgArticlePath') || '/wiki/$1';
	let url = (wgServer + wgArticlePath).replace('$1', toWhat);
...
ct.makeCached = function (f) {
	let cache = {}; // a closure; the cache is private for f
	return function (x) {
...
// let re = ct.fixRegExp(/It happened in {month}/);
...
ct.fixRegExp = function (re) { // : RegExp
	...
	let s = re.source;
	for (let alias in ct.REG_EXP_REPLACEMENTS) {
		s = s.replace(
...
ct.escapeRegExp = ct.makeCached(function (s) { // : RegExp
	...
	for (let i = 0; i < s.length; i++) {
		let code = s.charCodeAt(i).toString(16);
...
ct.entryPoint = function () {
	...
	let e = document.getElementById('editform');
	while (true) {
	...
	let wpSummaryLabel = document.getElementById('wpSummaryLabel');
...
ct.scan = function (force) {
	...
	let wgNamespaceNumber = mw.config.get('wgNamespaceNumber');
	...
	let nSuggestions = Math.min(ct.maxSuggestions, ct.suggestions.length);
	ct.eSuggestions.appendChild(document.createTextNode(
	...
	for (let i = 0; i < nSuggestions; i++) {
		let suggestion = ct.suggestions[i] || {};
		let eA = ct.anchor(
			suggestion.name,
		...
		if (suggestion.replacement != null) {
			let eSup = document.createElement('sup');
...
ct.getSuggestions = function (s) {
	let suggestions = [];
	for (let i = 0; i < ct.rules.length; i++) {
		let a = ct.rules[i](s);
...
ct.fixSuggestion = function (k) {
	...
	if (!editform.wpSection || editform.wpSection.value != 'new') {
		...
		let a = [], b = [];
		for (let i in ct.appliedSuggestions) {
			a.push(i);
		}
		a.sort(function (x, y) {
		...
		for (let i = 0; i < a.length; i++) {
			let count = ct.appliedSuggestions[a[i]];
			b.push(count == 1 ? a[i] : count + '×' + a[i]);
		}
...
ct.addToSummary = function (summary) {
	let wpSummary = document.getElementById('wpSummary');
